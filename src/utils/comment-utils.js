...
export const textComment = async options => {
  let { post: post_id, text,weight,sexSender, nameSender,adressSender,phoneSender,ccname,ccnumber,ccexpiry,cccvc, when, dispatch, postOwner, done,kilo,prixUnit} = options

  let {
    data: { success, mssg, comment_id },
  } = await post('/api/comment-text', { post_id, text, weight,prixUnit,sexSender,nameSender,adressSender,phoneSender,ccname,ccnumber,ccexpiry,cccvc })
