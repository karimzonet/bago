...
// User searching
new d('form.form_search').on('submit', e => {
  e.preventDefault()

  let departure = new d('.l_departure').val()
  let arrival = new d('.l_arrival').val()
  let departureDate_var = new d('.datepicker').val()
Notify({ value: departure})
Notify({ value: arrival})
Notify({ value: departureDate_var})
  // Date conversion with database format
  let departureDate = departureDate_var
    .split('-')
    .reverse()
    .join('/')
  ...
  let searchOpt = {
    ...
    btn: '.l_submit',
    url: '/user/searchPost',
    defBtnValue: 'Searching...',
  }
  console.log(searchOpt.data)
  User.commonSearch(searchOpt)
})
