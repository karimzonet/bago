...
app.get('/signup', mw.NotLoggedIn, (req, res) => {
  let options = { title: 'Inscrivez-vous gratuitement' }
...
const sendMailAndcreateDir = async (insertId, username, email, res) => {
  ...
  let url = `http://localhost:${
    ...
    options = {
      to: email,
      subject: 'Activer votre compte Bago',
      html: `<span>Bonjour ${username}, Vous recevez cet email car vous venez de créer un compte sur la plateforme Bago Online.<span><br><span>Cliquez sur le boutton ci-dessous pour activer votre compte et commmencer l'exploration.</span><br><br><a href='${url}' style='border: 1px solid #1b9be9; font-weight: 600; color: #fff; border-radius: 3px; cursor: pointer; outline: none; background: #1b9be9; padding: 4px 15px; display: inline-block; text-decoration: none;'>Activer</a>`,
    }
  ...
  try {
    ...
    res.json({
      mssg: `Bonjour, ${username}!!`,
      success: true,
    })
  } catch (error) {
    res.json({
      mssg: `Bonjour, ${username}. Le Mail ne peut pas être envoyé!!`,
      success: true,
    })
  }
}
