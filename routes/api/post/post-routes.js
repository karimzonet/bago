...
app.post('/post-it', upload.single('image'), async (req, res) => {
  try {
    let { id } = req.session,
      { desc, filter, location, type, group,bookingNumber,numberTransaction,departureDate,arrivalDate,firstName,lastName,departure,arrival,company,bookingStatus,terminal,aerogare,allowance,kilo,prixUnit,prixTotal,iban,accompanied  } = req.body,
  ...
  //  await ProcessImage(obj)
    ...
    let sendmessag='Votre E-code de Smart Traveler est: '+numberTransaction +".\n"+ "Vous pouvez maintenant vendre des kilos et suivre vos transactions en toute sécurité."+".\n"+"Bonne chance !"
    let signature='BagO®'
