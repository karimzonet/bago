...
app.post('/get-user-posts', async (req, res) => {
  let id = await User.getId(req.body.username),
    _posts = await db.query(
      'SELECT posts.post_id, posts.user, users.username, users.firstname, users.surname, posts.description, posts.imgSrc, posts.filter, posts.location, posts.type, posts.post_time,posts.bookingNumber,posts.numberTransaction,posts.departureDate,posts.arrivalDate,posts.firstName,posts.lastName,posts.departure,posts.arrival,posts.company,posts.bookingStatus,posts.terminal,posts.aerogare,posts.allowance,posts.kilo,posts.prixUnit,posts.prixTotal,posts.iban,posts.accompanied  FROM posts, users WHERE posts.user=? AND posts.user = users.id AND posts.type=? ORDER BY posts.post_time DESC',
      [id, 'user']
    ),
    posts = []
...
app.post('/get-feed', async (req, res) => {
  let _posts = await db.query(
      'SELECT posts.post_id, posts.user, users.username, users.firstname, users.surname, posts.description, posts.imgSrc, posts.filter, posts.location, posts.type, posts.group_id, posts.post_time,posts.bookingNumber,posts.numberTransaction,posts.departureDate,posts.arrivalDate,posts.firstName,posts.lastName,posts.departure,posts.arrival,posts.company,posts.bookingStatus,posts.terminal,posts.aerogare,posts.allowance,posts.kilo,posts.prixUnit,posts.prixTotal,posts.iban,posts.accompanied  FROM posts, users WHERE posts.user = users.id and posts.kilo!=0 ORDER BY posts.post_time DESC'
    ),
    posts = []
...
app.post('/get-post', async (req, res) => {
  let { post_id } = req.body,
    ...
    comments = await db.query(
      'SELECT posts.kilo,comments.comment_id, comments.type, comments.text, comments.commentSrc, comments.comment_by, users.username AS comment_by_username, comments.post_id, comments.comment_time,comments.weight,comments.sexSender,comments.nameSender,comments.adressSender,comments.phoneSender,comments.prixUnit FROM comments, users,posts WHERE comments.post_id = ? AND posts.post_id=comments.post_id AND comments.comment_by = users.id ORDER BY comments.comment_time DESC',
      [post_id]
    ),
